...
(select j, s_dest, x, likelihood, rank() over (partition by s_source order by likelihood desc) as ranking from (select Y.j, A.s_source, A.s_dest, x || array[A.s_source] as x, viterbi.likelihood*A.w*B.prob as likelihood from A
...
select j, s, x || array[s] as states, likelihood from viterbi
